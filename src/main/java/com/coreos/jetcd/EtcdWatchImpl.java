...
public class EtcdWatchImpl implements EtcdWatch {
  ...
  private ConcurrentLinkedQueue<Pair<WatcherImpl, SettableFuture<Watcher>>>
      pendingCreateWatchers = new ConcurrentLinkedQueue<>();
  private ConcurrentHashMap<Long, SettableFuture<Boolean>> pendingCancelFutures = new ConcurrentHashMap<>();
   ...
   */
  @Override
  public SettableFuture<Watcher> watch(ByteSequence key, WatchOption watchOption,
    ...
    SettableFuture<Watcher> waitFuture = SettableFuture.create();
   ...
   */
  protected SettableFuture<Boolean> cancelWatch(long id) {
    WatcherImpl temp = watchers.get(id);
    SettableFuture<Boolean> completableFuture = null;
    if (temp != null) {
      synchronized (temp) {
  ...
  private synchronized void resume() {
    ...
    this.watchers.clear();
    for (SettableFuture<Boolean> watcherCompletableFuture : pendingCancelFutures.values()) {
      watcherCompletableFuture.set(Boolean.TRUE);
    }
  ...
  private void processCreate(WatchResponse response) {
    ...
    if (response.getCreated()) {
      ...
      if (response.getWatchId() == -1 && watcher.callback != null) {
        ...
        requestPair.getValue().set(watcher);
      }
    ...
    if (watcher != null) {
      synchronized (watcher) {
        if (watchResponse.getEventsCount() != 0) {
          ...
          if (watcher.isResuming()) {
            final long lastRevision = watcher.getLastRevision();
            Iterables.removeIf(events, new Predicate<Event>() {

                @Override
                public boolean apply(Event e) {
                    return e.getKv().getModRevision() <= lastRevision;
                }
            });
          }
      ...
      // if the watcher is not canceling, cancel it.
        SettableFuture<Boolean> completableFuture = this.pendingCancelFutures
  ...
  private void processCanceled(WatchResponse response) {
      SettableFuture<Boolean> cancelFuture = this.pendingCancelFutures
        .remove(response.getWatchId());
    cancelFuture.set(Boolean.TRUE);
  }
  ...
  public class WatcherImpl implements Watcher {
    ...
    @Override
    public SettableFuture<Boolean> cancel() {
